...
def get_gcloud_credentials(scopes):
    creds_dict = dict(st.secrets["gcloud"][
                          "my_project_settings"])  # Make sure your service account JSON is stored in the secrets.toml file under the key "gcloud" and the subkey "my_project_settings"
    ...
    return service_account.Credentials.from_service_account_file(temp_path,
                                                                 scopes=scopes)  # Return a service account object for later use in authentication
'''
